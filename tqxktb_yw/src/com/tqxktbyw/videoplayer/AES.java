...
public class AES {
   ...
   /* public static void dump(String label, byte[] data)
    ...
    public void test(String OrgPAH,String DecPATH) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, ShortBufferException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException
    {
       // System.out.println("=======================AES/CBC/PKCS5Padding=====================");
        // key
        AesVideoDec aeskeyiv = new AesVideoDec();
        byte[] key = aeskeyiv.getkey().getBytes("UTF-8");
        //dump("key", key);
        // iv
        byte[] iv = aeskeyiv.getIv().getBytes("UTF-8");
       // dump("iv", iv);
       byte[] data = aeskeyiv.getData(OrgPAH);
        //System.out.println(aeskeyiv.getData("/home/mryys/yy.amt").length());
       // dump("传回数据：", data);
...
//        dump("outdata", outdata);
        ...
        byte[] indata1 = aes.decrypt(data);
      //  dump("indata1", indata1);
        aeskeyiv.setData(OrgPAH,DecPATH,indata1);
    }
